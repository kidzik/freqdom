...
#' frequency-domain methods such as \code{\link{invfourier}}.
...
#' \code{\link{invfourier}} or any of the \code{freqdom.*} functions.
#'
...
#' 
#' For an array \code{X} of dimensions \eqn{L \times p_1 \times p_2}, each element \code{X[i,,]} corresponds to freq \code{freq[i]} for \eqn{1 \leq i \leq L}.
#' 
#' @title Converts an array of filters into a frequency-domain object
#' 
#' @param X a set of \eqn{L} operators represented as an array of dimensions \eqn{L \times p_1 \times p_2}.
#' @param freq a vector of length \eqn{L} of frequencies on which the operators are defined. \code{freq[i]} corresponds to the operator \code{X[i,,]}.
#' @return A frequency domain operator represented as a list with elements
#' * \code{$operators} = \eqn{X},
#' * \code{$freq} = \code{freq}.
